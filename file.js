...
function fbutton(){
    ...
    container.innerHTML=`<div style="margin:1rem;font-family:sans-serif;font-size:1.5rem;">Pipes and Cisterns</div>`;
    container.innerHTML+=`<div style="text-align:center; font-size:2rem;border:2px solid rgb(31, 168, 231); 
    border-radius:10rem;width:3rem; height:3rem;position:stick" id="newTimer"></div>`
    var newTime=document.getElementById("newTimer");
    newTime.innerHTML=setInterval(function a(){
        
    var timer=500;
        timer--;
        newTime.innerText=timer;//problem here....
    if(timer===0){
    alert("Time's Up");
    console.log(timer);
    }
    ...
    container.innerHTML+=`<div style="text-align:right;margin:1rem;font-family:sans-serif;font-size:1.5rem">SCORE:
   <span style="font-weight:bold"> ${index}</span></div>`;
    ...
    container.innerHTML+=`<div  id="Question" style="margin:1rem;border:2px solid rgb(194, 175, 145);
    ...
    for(let i=0;i<currentquestion.option.length;i++){
        ...
        container.innerHTML+=`<button style="margin:1rem;text-align:left;border-color:rgb(31, 168, 231);
   ...
   container.innerHTML+=`<button id="nextquestion" style="margin:1rem;text-align:center;border-color:rgb(31, 168, 231);
    color:rgb(31, 168, 231);font-size:1rem;width:15rem;font-family:sans-serif;
   height:3rem;border-radius:0.5rem;background-color:white">Next Question</button>`
   
}
/*let nextques=document.getElementById("nextquestion");
nextques.addEventListener("click",shownext);
function shownext () {
    
    for(let i=0;i<pipes.question.length;i++){
        let element=pipes.question[i];
...
//https://github.com/Sh7909/quiz-assignment
//https://github.com/Sh7909/quiz-assignment.git
